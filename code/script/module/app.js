var App = function () {
    ...
    var handleResponsive = function () {
        ...
        if (jQuery.browser.msie && jQuery.browser.version.substr(0, 1) == 8) {
            isIE8 = true; // checks for IE8 browser version
            $('.visible-ie8').show(); //
        }
    ...
    var validation_option = {
        ...
        errorPlacement: function (errors, element) {
            ...
            var isChosen = isSelect && (element.hasClass("chosen") || element.hasClass("chosen-with-diselect"));
            var target = element;
            if (isChosen) {
                var isMultiple = element.attr("multiple") != null;
                if (!isMultiple) {
                    target = element.siblings(".chzn-container").find("a.chzn-single");
                } else {
                    target = element.siblings(".chzn-container").find("ul");
                }
            }
    ...
    var handleTooltip = function () {
        ...
        if (App.isTouchDevice()) { // if touch device, some tooltips can be skipped in order to not conflict with click events
            jQuery('.tooltips:not(.no-tooltip-on-touch-device)', ctx).tooltip();
        } else {
            jQuery('.tooltips', ctx).tooltip();
        }
    };

    var handlePopover = function () {
        var ctx = arguments.length > 0 ? arguments[0] : null;
        jQuery('.popovers', ctx).popover();
    };

    var handleChoosenSelect = function (ctx) {
        if (!jQuery().chosen) {
            return;
        }
    ...
    var handleDateTimePickers = function () {
        ...
        if (jQuery().datepicker) {
            ...
            for (var i = 0; i < datePickers.size(); i++) {
                ...
                datePicker.datepicker({format: dataDateFormat || dateFormat || "yyyy-mm-dd", weekStart: 1});
            }
        }
...
//            handleLazySection();
        ...
        blockUI: function (el, loaderOnTop) {
            lastBlockedUI = el;
            jQuery(el).block({
                message: '<img src="' + ctx + '/resources/common/static/img/loading.gif" align="absmiddle">',
                css: {
                    border: 'none',
                    padding: '2px',
